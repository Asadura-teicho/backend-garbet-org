...
// -------------------------------------------
// @desc    Get active game (accepting bets)
// @route   GET /api/dice-roll-games/active
// @access  Public
// -------------------------------------------
exports.getActiveGame = asyncHandler(async (req, res) => {
  const game = await DiceRollGame.findOne({ status: 'accepting-bets' })
    .populate('createdBy', 'username email')
    .sort({ gameNumber: -1 });
  ...
  if (!game) {
    return res.json({
      success: true,
      data: { game: null, message: 'No active game' },
    });
  }

  // Get bets for this game
  const bets = await DiceRollBet.find({ game: game._id, status: 'pending' })
    .populate('user', 'username email')
    .sort({ createdAt: -1 });
...
// -------------------------------------------
// @desc    Select winner for PvP game (admin override)
// @route   PATCH /api/dice-roll-games/:id/select-pvp-winner
// @access  Admin
// -------------------------------------------
exports.selectPvPWinner = asyncHandler(async (req, res) => {
  const { winner, player1Roll, player2Roll } = req.body;
  const adminId = req.user.id;
  const session = await mongoose.startSession();
  session.startTransaction();
  ...
  try {
    ...
    if (game.gameType !== 'player-vs-player') {
      await session.abortTransaction();
      throw new AppError('This endpoint is only for player-vs-player games', 400);
    }
    ...
    // Set winner and dice rolls (admin override)
    game.selectedWinner = winner;
    game.winnerSelectedBy = adminId;
    
    // Set dice rolls if provided
    if (player1Roll !== undefined) {
      if (player1Roll < 1 || player1Roll > 6) {
        await session.abortTransaction();
        throw new AppError('Player 1 dice roll must be between 1 and 6', 400);
      }
      game.players.player1.diceRoll = player1Roll;
    }
    
    if (player2Roll !== undefined) {
      if (player2Roll < 1 || player2Roll > 6) {
        await session.abortTransaction();
        throw new AppError('Player 2 dice roll must be between 1 and 6', 400);
      }
      game.players.player2.diceRoll = player2Roll;
    }

    // Set dice result for display
    const winnerRoll = winner === 'player1' 
      ? (game.players.player1.diceRoll || player1Roll || 1)
      : (game.players.player2.diceRoll || player2Roll || 1);
    game.diceResult = winnerRoll;

    // Process payouts if game wasn't already completed
    if (game.status !== 'completed') {
      ...
      const winAmount = totalPot; // Winner takes all

      const winnerUserDoc = await User.findById(winnerUser).session(session);
      if (winnerUserDoc) {
        winnerUserDoc.balance += winAmount;
        ...
        await Transaction.create([{
          ...
          currency: 'TRY',
          status: 'completed',
          description: `Dice Roll PvP Game #${game.gameNumber} - Admin Selected Winner`,
          metadata: { gameId: game._id, gameType: 'player-vs-player', adminSelected: true },
        }], { session });
      }
    ...
    res.json({
      success: true,
      message: 'PvP winner selected and payouts processed',
      data: { game },
    });
  } catch (error) {
    await session.abortTransaction();
...
exports.changeOutcome = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Winner is changing - need to reverse payouts and process new ones
    if (isPvP) {
      // For PvP games, reverse old winner payout and process new winner
      const oldWinnerUser = oldWinner === 'player1' ? game.players.player1.user : game.players.player2.user;
      const newWinnerUser = newWinner === 'player1' ? game.players.player1.user : game.players.player2.user;
      const totalPot = game.players.player1.betAmount + game.players.player2.betAmount;

      // Reverse old winner payout
      if (oldWinnerUser) {
        const oldWinnerDoc = await User.findById(oldWinnerUser).session(session);
        if (oldWinnerDoc) {
          oldWinnerDoc.balance -= totalPot;
          await oldWinnerDoc.save({ session });
        }
      }

      // Process new winner payout
      if (newWinnerUser) {
        const newWinnerDoc = await User.findById(newWinnerUser).session(session);
        if (newWinnerDoc) {
          newWinnerDoc.balance += totalPot;
          await newWinnerDoc.save({ session });
          ...
          await Transaction.create([{
            ...
            currency: 'TRY',
            status: 'completed',
            description: `Dice Roll PvP Game #${game.gameNumber} - Admin Changed Winner`,
            metadata: { gameId: game._id, gameType: 'player-vs-player', adminChanged: true },
          }], { session });
        }
      }
  ...
  try {
    ...
    if (game.players.player1.diceRoll !== null && game.players.player2.diceRoll !== null) {
      ...
      if (player1Roll > player2Roll) {
        ...
        // Tie - higher bet amount wins, or random if equal
        if (game.players.player1.betAmount > game.players.player2.betAmount) {
          winner = 'player1';
        } else if (game.players.player2.betAmount > game.players.player1.betAmount) {
          winner = 'player2';
        } else {
      ...
      // Only auto-determine winner if admin hasn't set it
      if (!game.selectedWinner) {
        game.selectedWinner = winner;
        game.status = 'completed';
        game.completedAt = new Date();
        game.diceResult = winner === 'player1' ? player1Roll : player2Roll;
        ...
        if (winnerUserDoc) {
          ...
          await Transaction.create([{
            ...
            currency: 'TRY',
            status: 'completed',
            description: `Dice Roll PvP Game #${game.gameNumber} - Win`,
            metadata: { gameId: game._id, gameType: 'player-vs-player' },
          }], { session });
        }
      } else {
        // Admin has set winner, just update dice result for display
        game.diceResult = winner === 'player1' ? player1Roll : player2Roll;
      }
    }
